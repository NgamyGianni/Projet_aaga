...
public class Stage1SecondaryA extends Brain {
  ...
  public void step() {
    ...
    if (state==MOVETASK) {
      ...
      sendLogMessage("Moving a head. Waza!");
      sendLogMessage("radar size returns value ("+(int)(int)detectRadar().size()+").");
      return;
    }
    if (state==MOVETASK && detectFront().getObjectType()==IFrontSensorResult.Types.WALL && detectRadar().get(0).getObjectDistance() < 10 ) {
      state=TURNRIGHTTASK;
      oldAngle=getHeading();
